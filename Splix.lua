...
local library = {
    ...
    changed = {},
    folders = {
        assets = "calypso/assets",
        configs = "calypso/configs"
    }
}
--
...
--local themes = {}
local theme = {
    accent = Color3.fromRGB(84, 41, 255),
	...
	function utility:Create(instanceType, instanceOffset, instanceProperties, instanceParent)
        ...
        if instance then
            ...
            if not instanceHidden then
                library.drawings[#library.drawings + 1] = {instance, instanceOffset, instance.Transparency}
            else
...
do
    ...
    function library:New(info)
        ...
        --
        local window = {pages = {}, isVisible = true, currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil}}
        --
        ...
        function window:Watermark(info)
            ...
            --
            local info = info or {}
            local watermark_name = info.name or info.Name or info.title or info.Title or ""
            --
            ...
            --
            function window.watermark:Fade(visibility)
                utility:Lerp(watermark_outline, {Transparency = visibility and 1 or 0}, 1)
                utility:Lerp(watermark_inline, {Transparency = visibility and 1 or 0}, 1)
                utility:Lerp(watermark_frame, {Transparency = visibility and 1 or 0}, 1)
                utility:Lerp(watermark_accent, {Transparency = visibility and 1 or 0}, 1)
                utility:Lerp(watermark_title, {Transparency = visibility and 1 or 0}, 1)
            end
            --
        ...
        function window:Notification(info)
            local notification = {stack = #notifications+1}
            --
            local info = info or {}
            local notification_name = info.name or info.Name or info.title or info.Title or ""
            --
            local text_bounds = utility:GetTextBounds(notification_name, theme.textsize, theme.font)
            --
            local notification_outline = utility:Create("Frame", {Vector2.new(-text_bounds.X-20,38/4+27*notification.stack)}, {
                Size = utility:Size(0, text_bounds.X+20, 0, 21),
                Position = utility:Position(0, -text_bounds.X-20, 0, 38/4+27*notification.stack),
                Transparency = 1,
                Hidden = true,
                ZIndex = 60,
                Color = theme.outline,
            ...
            --
            local notification_inline = utility:Create("Frame", {Vector2.new(1,1), notification_outline}, {
                Size = utility:Size(1, -2, 1, -2, notification_outline),
                Position = utility:Position(0, 1, 0, 1, notification_outline),
                Transparency = 1,
                Hidden = true,
                ZIndex = 60,
                Color = theme.inline,
            ...
            --
            local notification_frame = utility:Create("Frame", {Vector2.new(1,1), notification_inline}, {
                Size = utility:Size(1, -2, 1, -2, notification_inline),
                Position = utility:Position(0, 1, 0, 1, notification_inline),
                Transparency = 1,
                Hidden = true,
                ZIndex = 60,
                Color = theme.light_contrast,
            ...
            --
            local notification_accent = utility:Create("Frame", {Vector2.new(0,0), notification_frame}, {
                Size = utility:Size(0, 1, 0, notification_frame.Size.Y, notification_frame),
                Position = utility:Position(0, 0, 0, 0, notification_frame),
                Transparency = 1,
                Hidden = true,
                ZIndex = 60,
                Color = theme.accent,
            ...
            --
            local notification_title = utility:Create("TextLabel", {Vector2.new(2 + 6,4), notification_outline}, {
                ...
                Hidden = true,
                ZIndex = 60,
                Position = utility:Position(0, 2 + 6, 0, 4, notification_outline),
                Transparency = 1,
                Visible = true
            })
            --
            function notification:Fade(visibility)
                utility:Lerp(notification_outline, {Transparency = visibility and 1 or 0}, 0.3)
                utility:Lerp(notification_inline, {Transparency = visibility and 1 or 0}, 0.3)
                utility:Lerp(notification_frame, {Transparency = visibility and 1 or 0}, 0.3)
                utility:Lerp(notification_accent, {Transparency = visibility and 1 or 0}, 0.3)
                utility:Lerp(notification_title, {Transparency = visibility and 1 or 0}, 0.3)
            end
            --
            function notification:Move(vector, speed)
                utility:Lerp(notification_outline, {Position = utility:Position(0, vector.X, 0, vector.Y)}, speed)
                utility:Lerp(notification_inline, {Position = utility:Position(0, vector.X+1, 0, vector.Y+1)}, speed)
                utility:Lerp(notification_frame, {Position = utility:Position(0, vector.X+2, 0, vector.Y+2)}, speed)
                utility:Lerp(notification_accent, {Position = utility:Position(0, vector.X+2, 0, vector.Y+2)}, speed)
                utility:Lerp(notification_title, {Position = utility:Position(0, vector.X + (2 + 6), 0, vector.Y+4)}, speed)
            end
            --
            function notification:Remove()
                utility:Remove(notification_outline, true)
                utility:Remove(notification_inline, true)
                utility:Remove(notification_frame, true)
                utility:Remove(notification_accent, true)
                utility:Remove(notification_title, true)
                --
            ...
            function notification:UpdateSize()
                notification_outline.Size = utility:Size(0, notification_title.TextBounds.X + 4 + (6*2), 0, 21)
                notification_inline.Size = utility:Size(1, -2, 1, -2, notification_outline)
                notification_frame.Size = utility:Size(1, -2, 1, -2, notification_inline)
                notification_accent.Size = utility:Size(0, 1, 0, notification_frame.Size.Y, notification_frame)
            end
            --
            function notification:UpdatePosition(xPos, yPos)
                notification_outline.Position = utility:Position(0, xPos, 0, yPos)
                notification_inline.Position = utility:Position(0, 1, 0, 1, notification_outline)
                notification_frame.Position = utility:Position(0, 1, 0, 1, notification_inline)
                notification_accent.Position = utility:Position(0, 0, 0, 0, notification_frame)
                notification_title.Position = utility:Position(0, 2 + 6, 0, 4, notification_outline)
            end
            --
            notification:UpdateSize()
            --
            notifications[notification.stack] = notification
            --
            notification:Fade(false)
            --
            wait(0.5)
            --
            notification:UpdatePosition(0, 38/4+27*notification.stack)
            --
            spawn(function()
                spawn(function()
                    notification:Move(utility:Position(0, 100, 0, notification_outline.Position.Y), 0.2)
                    wait(0.21)
                    notification:UpdatePosition(100, notification_outline.Position.Y)
                end)
                --
                ...
                --
                spawn(function()
                    notification:Move(utility:Position(0, 0, 0, notification_outline.Position.Y), 0.2)
                end)
                --
                notification:Fade(false)
                --
        ...
        function window:Cursor(info)
            ...
            utility:Connection(rs.RenderStepped, function()
                ...
                cursor.PointB = Vector2.new(mouseLocation.X + 10, mouseLocation.Y + 4)
                cursor.PointC = Vector2.new(mouseLocation.X + 4, mouseLocation.Y + 10)
                --
                cursor_inline.PointA = Vector2.new(mouseLocation.X - 3, mouseLocation.Y - 3)
                cursor_inline.PointB = Vector2.new(mouseLocation.X + 13, mouseLocation.Y + 3)
                cursor_inline.PointC = Vector2.new(mouseLocation.X + 3, mouseLocation.Y + 13)
            end)
            --
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.KeyCode == Enum.KeyCode.RightShift then
                ...
                spawn(function()
                    for i, v in pairs(library.drawings) do
                        utility:Lerp(v[1], {Transparency = window.isVisible and v[3] or 0}, 0.2)
                    end
                end)
                --
    ...
    function sections:Toggle(info)
        ...
        function toggle:Colorpicker(info)
            ...
            local colorpicker = {axis = toggle.axis, index = toggle.colorpickers, current = {hh, ss, vv , (transp or 0)}, holding = {picker = false, huepicker = false, transparency = false}, holder = {inline = nil, picker = nil, picker_cursor = nil, huepicker = nil, huepicker_cursor = {}, transparency = nil, transparencybg = nil, transparency_cursor = {}, drawings = {}}}
            --
    ...
    function sections:Textbox(info)
        ...
        local pointer = info.pointer or info.Pointer or info.flag or info.Flag or nil
        local callback = info.callback or info.callBack or info.Callback or info.CallBack or function(text)end
        --
        local window = self.window
        local page = self.page
        local section = self
        --
        local textbox = {focused = false, current = {tostring(def)}, axis = section.currentAxis}
        --
        ...
        --
        local textbox_value = utility:Create("TextLabel", {Vector2.new((math.sqrt(theme.textsize)-0.1)*string.len(def)+3,1), textbox_frame}, {
            Text = def,
            Size = theme.textsize,
            Font = theme.font,
            Color = theme.textcolor,
            OutlineColor = theme.textborder,
            Center = true,
            Position = utility:Position(0, 3, 0, 1, textbox_frame),
            Visible = page.open
        }, section.visibleContent)
        --
        utility:LoadImage(textbox_gradient, "gradient", "https://i.imgur.com/5hmlrjX.png")
        --
        function textbox:Update(value)
            local value = value or 1
            --
            textbox_value.Position = textbox_value.Position + (Vector2.new(math.sqrt(theme.textsize)-0.1, 0)*value)
            utility:UpdateOffset(textbox_value, {Vector2.new((math.sqrt(theme.textsize)-0.1)*string.len(textbox_value.Text)+3,1), textbox_frame})
        end
        --
        function textbox:Set(value)
            if typeof(value) == "string" then
                ...
                textbox:Update(string.len(value))
                --
        ...
        library.began[#library.began + 1] = function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                if textbox_outline.Visible and window.isVisible and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + textbox.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + textbox.axis + 20}) and not window:IsOverContent() then
                    textbox.focused = true
                elseif textbox.focused then
                    textbox.focused = false
                    textbox.current = textbox_value.Text
                    --
                    callback(textbox.current)
                end
            elseif Input.UserInputType == Enum.UserInputType.Keyboard and textbox.focused then
                if Input.KeyCode == Enum.KeyCode.Return then
                    ...
                    --
                    callback(textbox.current)
                elseif Input.KeyCode == Enum.KeyCode.Backspace and textbox_value.Text ~= "" then
                    textbox_value.Text = textbox_value.Text:sub(0, -2)
                    textbox_value.Position = textbox:Update(-1)
                else
                    local allowed = false
                    local character = nil
                    --
                    for i,v in pairs(allowedCharacters) do
                        if i == Input.KeyCode.Name then
                            allowed = true
                            character = v:lower()
                        elseif v == Input.KeyCode.Name then
                            allowed = true
                            character = v:lower()
                        end
                    end
                    ...
                    if allowed then
                        ...
                        --
                        textbox_value.Text = textbox_value.Text..character
                        textbox:Update()
                    end
                end
            end
    ...
    function sections:Dropdown(info)
        ...
        local section = self
        --
        local dropdown = {open = false, current = tostring(def), options = options, holder = {buttons = {}, drawings = {}}, axis = section.currentAxis}
        --
        ...
        function dropdown:Set(value)
            if typeof(value) == "string" and table.find(dropdown.options, value) then
                ...
                elseif utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + dropdown.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + dropdown.axis + 15 +  20}) and not window:IsOverContent() then
                    if not dropdown.open then
                        ...
                        local dropdown_open_outline = utility:Create("Frame", {Vector2.new(0,19), dropdown_outline}, {
                            Size = utility:Size(1, 0, 0, 3 + (#dropdown.options * 19), dropdown_outline),
