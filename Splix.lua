...
do
    ...
    function library:New(info)
        ...
        function window:Watermark(info)
            ...
            --
            local info = info or {}
            local watermark_name = info.name or info.Name or info.title or info.Title or string.format("Gameover || ping : %u || fps : %u", 1, 100, 200)
            --
            ...
            function window.watermark:Update(updateType, updateValue)
                if updateType == "Visible" then
                    print(updateValue, "WATERMARK")
                    window.watermark.visible = updateValue
                    window.watermark:Visibility()
                end
            end
            watermark_title.Text = string.format("Gameover || ping : %i || fps : %u", 1, tostring(library.shared.ping), library.shared.fps)
